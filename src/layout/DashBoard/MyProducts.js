...
const MyProducts = () => {
    ...
    const { data: items = [], isLoading, refetch } = useQuery({
        queryKey: ['items'],
        queryFn: async () => {
            const res = await fetch(`https://simora-motors-server-mostafizurhh.vercel.app/advertised?email=${user?.email}`, {
                headers: {
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            fetch(`https://simora-motors-server-mostafizurhh.vercel.app/advertised/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
