...
const MyOrders = () => {
    ...
    const { data: bookings = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://simora-motors-server-mostafizurhh.vercel.app/bookings?email=${user?.email}`, {
                headers: {
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            fetch(`https://simora-motors-server-mostafizurhh.vercel.app/bookings/${id}`, {
                method: 'DELETE',
                headers: {
                    authorization: `bearer ${localStorage.getItem('accessToken')}`
                }
