...
const AdvertisedItem = ({ item, setAdvertiseItem }) => {
    ...
    if (saleStatus === 'Available') {
        ...
        return (
            <div className="border flex flex-col rounded-lg bg-base-100 shadow-lg mb-8 mt-8">
                <PhotoView src={image}>
                    <figure><img src={image} alt="Cars" style={{ height: 250, borderRadius: 30 }} className='p-4' /></figure>
                </PhotoView>
                <div className='px-3'>
                    <div className="h-[20px] font-bold text-primary text-start">{name}</div>
                    ...
                    <div className='flex flex-col pt-2'>
                        <p className='text-start font-bold text-primary animate-pulse'>For Sale: ${resale}</p>
                    </div>
